...
### This preprocesses the image and feeds it to the model
def preprocess_and_predict(img, model, class_names):
  ...
  common_name = class_names[predicted_class]
  print('common_name', common_name)
  return common_name, confidence

### This gets the plant info using the common name generated by the model 
def search_by_name(common_name,key):
  search_url = f"https://plant.id/api/v3/kb/plants/name_search?q={common_name}&thumbnails=true"
  print(search_url)
  headers = {
      'Api-Key': key,
...
### This scrapes the uses of the plant using the common name generated by the model
def search_plant_uses(common_name, botanical_name, family):
  ...
  if plant_uses is None:
    ...
    plant_uses = get_google_uses(common_name)
  return plant_uses
